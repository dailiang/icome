...
def evaluate(learning_rate=0.001, 
    ...
    layer3 = LogisticRegression(input=layer3_input, n_in=256, n_out=2)

    cost = layer3.negative_log_likelihood(LABEL)

    valid_model = theano.function([Y, UV, LABEL], layer3.accuracy(LABEL))
...
#          (epoch, this_valid_accuracy * 100))
    ...
    while 1:
        ...
        print '... save params'
        save_file = open(save_path + 'shared' + str(epoch) + '.pkl', 'wb')
        cPickle.dump(params, save_file, -1)
        save_file.close()

        save_file = open(save_path + str(epoch) + '.pkl', 'wb')
