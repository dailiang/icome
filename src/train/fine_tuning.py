...
def evaluate(learning_rate=0.001, 
    ...
    W_3 = theano.shared(value=cPickle.load(fle), borrow=True)
    b_3 = theano.shared(value=cPickle.load(fle), borrow=True)
    W_4 = theano.shared(value=cPickle.load(fle), borrow=True)
    b_4 = theano.shared(value=cPickle.load(fle), borrow=True)
    fle.close()
    ...
    params = layer4.params + layer3.params + layer2.params + layer1.params + \
            layer0_y.params + layer0_uv.params
    ...
    while 1:
        ...
        print '... save params'
        save_file = open(save_path + 'shared-ft-' + str(epoch) + '.pkl', 'wb')
        cPickle.dump(params, save_file, -1)
        save_file.close()

        save_file = open(save_path + 'ft' + str(epoch) + '.pkl', 'wb')
